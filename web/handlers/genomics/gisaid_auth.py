...
def gisaid_authorized(
    ...
    async def wrapper(self: RequestHandler, *args, **kwargs) -> Optional[Awaitable[None]]:
        try:
            ...
            self.set_status(401)
            self.add_header("WWW-Authenticate", 'Bearer realm="GISAID Authentication-Token"')
            return self.finish()
        ...
        token_diff_time = (
            dt.now(timezone.utc)
            - dt.utcfromtimestamp(decoded_token["last_checked"]).replace(tzinfo=timezone.utc)
        ).seconds
        ...
        if token_diff_time <= CACHE_TIME:  # Cached token
            ...
            token_diff_time <= CACHE_TIME * 2
        ):  # Cache expired. Extend cache time to allow user to get new token
            if decoded_token["is_authenticated"]:  # Authenticated
                reset_last_checked = True
        if reset_last_checked:
            ...
            resp = await _gisaid_gps_api_http_client.post(GPS_API_ENDPOINT, json=request_params)
